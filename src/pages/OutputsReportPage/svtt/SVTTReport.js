...
import { changeSVTT, fetchSVTTReportList } from "./svttReportSlice";
...
export default function SVTTReport() {
  const data = useSelector(getSVTTReport);
  const month = new Date().getMonth();
  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(fetchSVTTReportList(month + 1));
  }, [month]);
  const handleInputChange = (row, e) => {
    const newValue = { ...row, note: e.target.value };
    dispatch(changeSVTT(newValue));
  };
