...
import {
  deleteDetailSVTTReport,
  fetchDetailSVTTReport,
  searchDetailSVTTReport,
  updateDetailSVTTReport,
} from "./detailSVTTReportSlice";
import { getNumPageSVTT } from "redux/selectors";
...
function WrapTable({ columns, setData, data }) {
  ...
  const handleUpdateItem = (value) => {
    dispatch(updateDetailSVTTReport(value));
  };
  ...
  const [deleteSVTTs, setDeleteSVTTs] = useState([]);
  const dispatch = useDispatch();
  const location = useLocation();

  const handleDeleteSVTTs = () => {
    if (deleteSVTTs.length <= 0) {
      Swal.fire({
        icon: "error",
        title: "Oops...",
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          const value = [];
          deleteSVTTs.forEach((item) => {
            value.push(item.id);
          });
          dispatch(deleteDetailSVTTReport(value));
        }
      });
    }
  };
  const numPageList = useSelector(getNumPageSVTT);
  ...
  const handleInputChange = (e) => {
    ...
    dispatch(searchDetailSVTTReport({ outputId: location?.state?.id, value }));
  };
  ...
  const handleGotoPreviousPage = () => {
    dispatch(
      fetchDetailSVTTReport({ id: location?.state?.id, page: 0, num: pageSize })
    );
    setPageIndex(0);
  };
      ...
      <div className="d-flex flex-row align-items-center justify-content-between pb-3">
        ...
        <div>
          <Button
            ...
            onClick={handleDeleteSVTTs}
          >
            <i className="fas fa-trash-alt"></i>
            <span>DELETE</span> {deleteSVTTs?.length}
          </Button>
        </div>
      </div>
...
export default function DetailSVTTReport() {
  ...
  useEffect(() => {
    if (location?.state?.id) {
      dispatch(
        fetchDetailSVTTReport({ id: location?.state?.id, page: 0, num: 10 })
      );
    }
  }, [location?.state?.id]);
  useEffect(() => {
    setData(detailSVTTReport);
  }, [detailSVTTReport]);
